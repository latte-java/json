...
public final class JSONProcessor extends AbstractProcessor {
  ...
  void generateCompanion(TypeElement record, ModuleElement module) {
    ...
    enumImports.forEach(enumFqn -> sb.append("import ").append(enumFqn).append(";\n"));
    sb.append("\n");
  ...
  /**
   * Collects the fully-qualified names of every enum referenced anywhere in {@code t} — as the type
   * itself or as a {@code List}/{@code Set} element or {@code Map} key/value — into {@code into}. The
   * generated code emits enum types by simple name, so each must carry a matching import.
   */
